...
bool QtAxExcelEngine::setColumnCell(int rowNo, int columnNo, int rowCount, QVariant dataList)
{
    QAxObject *range = active_sheet_->querySubObject("Range(const QString&, const QString&)",
                                                     QtAxExcelEngine::cellsName(rowNo, columnNo),
                                                     QtAxExcelEngine::cellsName(rowNo + rowCount, columnNo));
    if (!range)
    {
        return false;
    }
    range->dynamicCall("SetValue(const QVariant&)", dataList);
    return  true;
